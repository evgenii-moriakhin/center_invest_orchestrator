...
class AppRunner:
    def __init__(self, app_image, app_port, healthcheck_api, app_dockerfile, app_git_repo):
    ...
    def start(self):
        ...
        if existing_container:
            logging.info(f"Container with name {self.app_image} already exists. Stopping and removing ...")
        ...
        self.container = self.client.containers.run(self.app_image, name=self.app_image, detach=True,
    ...
    def build_image(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            logging.info(f"Build context for app image is {build_context}, dockerfile is {dockerfile}")
            self.client.images.build(path=build_context, tag=self.app_image, dockerfile=dockerfile, rm=True)
    ...
    def get_existing_container(self, only_running: bool=False):
        logging.info(f"Checking for existing running container with name {self.app_image}")
        containers = self.client.containers.list(filters={"name": self.app_image}, all=not only_running)
        if containers:
    ...
    def get_status(self):
        ...
        if self.healthcheck_api:
            try:
                response = requests.get(f"http://localhost:{self.app_port}{self.healthcheck_api}", timeout=5)
    ...
    def get_cpu_usage(self):
        ...
        if system_cpu_delta > 0 and cpu_delta > 0:
            cpu_usage_percent = (cpu_delta / system_cpu_delta) * len(cpu_usage.get("percpu_usage", [])) * 100
